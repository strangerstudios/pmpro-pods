...
function pmpro_pods_permissions_user_has_permission( $user_has_permission, $object, $user ) {
	...
	if ( pmpro_pods_is_checkout_membership_level_restricted( $object ) ) {
		// Restrict if we are not on the correct membership level.
		return ! pmpro_pods_is_checkout_membership_level_restricted_for_user( $object, $user );
	}
...
function pmpro_pods_is_checkout_membership_level_restricted_for_user( $object, $user ) {
	...
	if ( ! pmpro_is_checkout() ) {
		...
		if ( ! pmpro_pods_is_membership_level_restricted_for_user( $object, $user ) ) {
			return false;
		}
...
function pmpro_pods_get_checkout_membership_levels( $object ) {
	...
	return pmpro_pods_get_required_membership_levels( $object );
}
